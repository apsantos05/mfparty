...
test('restaura masculino R$ 60 mais taxa e preserva compras temporárias assinadas',async()=>{
 const result=await call(create,{ticket:'homem',quantity:1,customer,request_id:'f1234567-1234-4234-8234-123456789014'});
 assert.equal(result.statusCode,200);assert.equal(result.body.amount_cents,6449);assert.equal(result.body.subtotal_cents,6000);assert.equal(result.body.service_fee_cents,449);
 for(const [ticket,cents] of [['homem',100],['homem',549],['mulher',4000],['jovem',2500]]){
 const old={v:1,id:'tx_oldprice',ref:'hp10_'+ticket+'_'+'e'.repeat(32),ticket,quantity:1,name:'Teste',expires:Date.now()+86400000,amount:cents};
 assert.ok(verify(sign(old)));
