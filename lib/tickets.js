...
function configurationErrors() {
  const errors=[];
  if(!redis.configured())errors.push('UPSTASH_REDIS_REST_URL / UPSTASH_REDIS_REST_TOKEN');
  if(signingSecret().length<32)errors.push('ORDER_SIGNING_SECRET (32 caracteres ou mais), ou TICKET_SIGNING_SECRET');
  if(!process.env.BRAVOPAY_API_KEY)errors.push('BRAVOPAY_API_KEY');
  if(!Number.isFinite(expiry()))errors.push('TICKET_VALID_UNTIL (data válida)');
  return errors;
}
function ready() { return configurationErrors().length===0; }
...
module.exports = {EVENT, key, expiry, ready, validCode, issue, check, configurationErrors};
