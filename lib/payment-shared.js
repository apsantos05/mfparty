...
const FEE=0;
const TICKETS=Object.freeze({mulher:{label:'Mulher',cents:4000,admissions:1},homem:{label:'Homem',cents:100,admissions:1},jovem:{label:'+16 anos · Open bar sem álcool (refrigerante, água e energético)',cents:2500,admissions:1},combo:{label:'Combo Amigo · Unissex',cents:8000,admissions:2}});
...
function validOrderAmount(order){
 const current=orderAmount(order);
 if(current===null)return false;
 if(current===order.amount)return true;
 if(order.amount===current+449*TICKETS[order.ticket].admissions*order.quantity)return true;
 // Preserva cobranças masculinas antigas, apenas quando o token já foi assinado pelo servidor.
 if(order.ticket!=='homem'||current===null)return false;
 const coupon=order.coupon||'';
 const rate=['DOLCE10','MARIF10','BRUNOJ10','PROMO10'].includes(coupon)?10:(normalizeCoupon(coupon)?25:0);
 const subtotal=6000*order.quantity;
 return order.amount===subtotal-Math.round(subtotal*rate/100)+449*order.quantity;
}
