...
function validOrderAmount(order){
 ...
 // Somente referências assinadas podem manter os preços temporários anteriores.
 const coupon=order.coupon||'';
 const rate=['DOLCE10','MARIF10','BRUNOJ10','PROMO10'].includes(coupon)?10:(normalizeCoupon(coupon)?25:0);
 const oldPrices=order.ticket==='homem'?[100]:[TICKETS[order.ticket].cents];
 return oldPrices.some(price=>{
  const subtotal=price*order.quantity;
  const discounted=subtotal-Math.round(subtotal*rate/100);
  const fees=order.ticket==='homem'?[0,449]:[0];
  return fees.some(fee=>order.amount===discounted+fee*TICKETS[order.ticket].admissions*order.quantity);
 });
}
